      ...
      <div className="w-full max-w-5xl mt-16">
        {records.map((record) => (
          <div key={record.id} className="mb-16">
            <p className="text-xs mb-2 text-purple-200">{record.pubDate?.toDateString()}</p>
            <h2 className="text-2xl mb-2">
              <a href={`posts/${record.slug}`}>{record.title}</a>
            </h2>
            <p className="text-purple-200 mb-5">{record.description}</p>
            <a href={`posts/${record.slug}`} className="px-4 py-2 font-semibold text-sm bg-purple-700 text-white rounded-lg shadow-sm w-fit">Read more &rarr;</a>
          </div>
        ))}
      </div>
